      ...
      <div className="flex flex-wrap justify-center  items-center gap-9 py-4">
        {!meal || meal.lenght == 0 ? (
          ...
          meal.map((item, index) => (
            <div
              key={index}
              className="card  overflow-hidden rounded-2xl  w-70 shadow-lg hover:shadow-2xl transition-all duration-300"
            >
              <div className="h-auto w-72">
                <figure>
                  <img src={item.image} alt="Food" className="w-full h-70" />
                </figure>
              </div>
              <div className="card-body bg-white text-black overflow-hidden items-center text-center gap-2.5">
                <h2 className="card-title text-nowrap text-sm  uppercase ">
                  {item.name}
                </h2>
                <div className="card-actions justify-end">
                  <NavLink to={`/${item.id}`}>
                    <button className="btn btn-primary pl-8 pr-8 rounded-3xl">
                      View
                    </button>
                  </NavLink>
                </div>
